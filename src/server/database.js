...
export const withDatabase = async (databaseFunction, operationName = 'Datenbankoperation') => {
  ...
  try {
    ...
    console.log(`Datenbankverbindung für ${operationName} hergestellt`);
    
    // Führe die übergebene Funktion aus
    const result = await databaseFunction(connection);
    console.log(`${operationName} erfolgreich abgeschlossen`);
    
    return result;
  } catch (error) {
    console.error(`Fehler bei ${operationName}:`, error);
    throw error;
  } finally {
    // Verbindung zurückgeben
    if (connection) {
      connection.release();
      console.log(`Datenbankverbindung für ${operationName} freigegeben`);
    }
  }
};
